...
class KeepAliveHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b"SYSTEM ACTIVE")
...
# --- 🌪️ المنظومة الآلية ---
def auto_engine():
    bot.send_message(CHAT_ID, "🦅 <b>المنظومة نشطة يا قائد هاني.. في وضع القتال!</b>", parse_mode='HTML')
    while True:
...
def auth(m):
    ...
    if m.text == PASSWORD:
        is_authorized = True
        bot.reply_to(m, "🦅 <b>مرحباً يا قائد هاني دوحة.. البوت الأسطوري جاهز!</b>", parse_mode='HTML')
